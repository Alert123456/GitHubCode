...
filename = 'data/unknow_city1.csv'
# 打印出csv文件的第一行
with open(filename) as f_obj:
    ...
    for row in reader:  # 遍历每一行
        ...
        if row[20] != "" :  # 最后一行为空字符，需特殊处理
            high = float(row[20].strip())
        if row[22] != "":  # 最后一行为空字符，需特殊处理
            low = float(row[22].strip())
        if low < 100 and high < 100:
            highs.append(high)  # 取每行第20列最高温度
            lows.append(low)  # 取每行第22列最低温度
            dates.append(current_date)
